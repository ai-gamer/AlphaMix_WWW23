...
import numpy as np
import math
import alphamix_ensemble_layer as be
...
class BE_clf(torch.nn.Module):
    def __init__(self, input_size, n_hidden,ens_size):
        super(BE_clf, self).__init__()
        self.input_size = input_size
        self.n_hidden = n_hidden
        self.ens_size = ens_size
        self.act = torch.nn.LeakyReLU()
        self.be1 = be.BatchEnsemble(self.input_size, self.n_hidden,ensemble_size=self.ens_size)
        self.be2 = be.BatchEnsemble(self.n_hidden, self.n_hidden,ensemble_size=self.ens_size)
        self.linear = nn.Linear(self.n_hidden, 2)
    
    def forward(self, x):
        x = x.reshape((x.shape[0], -1))
        x = self.act(self.be1(x))
        x = self.act(self.be2(x))
        x = self.linear(x)
        return x.squeeze()