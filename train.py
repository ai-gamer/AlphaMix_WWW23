...
def fit_base_model(data, net, criterion, optimizer, batch_size, train_episodes, dataset, model, seed, mode, device):
    ...
    for i in range(train_episodes):
        ...
        for b in range(0, len(y_train), batch_size):
            ...
            if train_batch_X.shape[0] == batch_size:
                ...
                if int(b / batch_size) % 100 == 0:
                    with torch.no_grad():
                        ...
                        for m in range(0, len(y_val), batch_size):
                            ...
                            if val_batch_X.shape[0] == batch_size:
...
def train_router(data, net_lst, batch_size, model_name, dataset, device, episodes, optimizer, router_net, bce_weight):
    X_train, y_train = data['X_train'], data['y_train']
    X_val, y_val = data['X_val'], data['y_val']
    ...
    for epi in range(episodes):
        ...
        for b in range(0, len(y_train['label']), batch_size):
            ...
            output_label, output_target = [],[]
            ensemble =4
            data_x_date_dup = torch.cat(ensemble*[batch_X])
            output_lst = []
            for net in net_lst:
            ...
            # output_label = torch.mean(torch.stack(output_lst), dim=0).squeeze()
            ...
            for i in range(len(output_label)):
                batch_router_x = (torch.mean(torch.stack(output_label[:i+1]),dim=0)*20).to(device)
        ...
        if not os.path.exists('./router/{}/{}/{}/'.format(dataset,model_name,bce_weight)):
            os.makedirs('./router/{}/{}/{}/'.format(dataset,model_name,bce_weight))
        torch.save(router_net, 'router/{}/{}/{}/router_epoch{}.pth'.format(dataset,model_name, bce_weight, epi))